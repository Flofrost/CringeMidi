...
### Generation ###
def generateNote(lengthInterval: list[int, int] = list([1, 30]), notePool: list[str] = noteTable, volumePool: list[str] = volumeTable) -> str:
    return \
        f"{randint(lengthInterval[0], lengthInterval[1])}" + \
        notePool[randint(0, len(notePool) - 1)] + \
        volumePool[randint(0, len(volumePool) - 1)]

def generateNotes(amount: int = 10, lengthInterval: list[int, int] = list([1, 30]), notePool: list[str] = noteTable, volumePool: list[str] = volumeTable):
    notes = list()
    for _ in range(amount):
        notes.append(generateNote(lengthInterval, notePool, volumePool))
    return notes
### Generation ###
